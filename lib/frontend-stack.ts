...
function handler(event) {
  ...
  headers['content-security-policy'] = {
    value: "default-src 'self'; script-src 'self'; style-src 'self'; img-src 'self' data: https://image.tmdb.org; font-src 'self' data:; connect-src 'self' https://api.DOMAIN_PLACEHOLDER; frame-ancestors 'none'; base-uri 'self'; form-action 'self'; object-src 'none';"
  };
  ...
  return response;
          ...
          `.replace(/DOMAIN_PLACEHOLDER/g, domainName),
        ),

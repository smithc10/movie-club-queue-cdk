...
3. **Deploy website stack** (CloudFront, S3, Route 53, ACM):
   ```bash
   npx cdk deploy MovieClubWebsiteStack
   ```
...
4. **Deploy API stack** (API Gateway, Lambda, DynamoDB):
   ```bash
   npx cdk deploy MovieClubApiStack
   ```
...
npx cdk diff MovieClubWebsiteStack
npx cdk diff MovieClubApiStack
